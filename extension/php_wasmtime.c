...
static wasm_trap_t* php_host_func_callback(
    ...
    if (nresults > 0) {
        if (Z_TYPE(php_retval) == IS_LONG) {
            ...
            // we assume a 32-bit for demonstration:
            results[0].of.i32 = (int32_t) Z_LVAL(php_retval);
        } else if (Z_TYPE(php_retval) == IS_DOUBLE) {
            results[0].kind = WASMTIME_F64;
            results[0].of.f64 = Z_DVAL(php_retval);
        } else {
            results[0].kind = WASMTIME_I32;
            results[0].of.i32 = 0; /* default */
        }
    }
            ...
            case WASM_EXTERN_FUNC: {
                if (found && (Z_TYPE_P(found) == IS_OBJECT || Z_TYPE_P(found) == IS_CALLABLE)) {
                    ...
                    if (zend_is_callable(found, 0, NULL)) {
                        // Build signature from the import's functype if needed. We'll skip signature checks here.
                        const wasm_functype_t *ftype = wasm_externtype_as_functype_const(et);

                        wasm_functype_t *clone = wasm_functype_copy(ftype);
